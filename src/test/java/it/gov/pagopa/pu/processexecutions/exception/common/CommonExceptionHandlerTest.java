...
import it.gov.pagopa.pu.processexecutions.exception.ProcessExecutionsExceptionHandler;
import it.gov.pagopa.pu.processexecutions.exception.transcoder.handler.ConstraintViolationExceptionMessageTranscoderTest;
import it.gov.pagopa.pu.processexecutions.util.TestUtils;
import it.gov.pagopa.pu.processexecutions.util.UtilitiesTest;
...
import static it.gov.pagopa.pu.processexecutions.exception.transcoder.handler.ConstraintViolationExceptionMessageTranscoderTest.EXPECTED_CONSTRAINT_EXCEPTION_MESSAGE_TRANSCODED;
...
public abstract class CommonExceptionHandlerTest {
  ...
  void handleForbiddenErrorExceptionError() throws Exception {
    ...
    performRequest(DATA, MediaType.APPLICATION_JSON)
      ...
      .andExpect(MockMvcResultMatchers.jsonPath("$.category").value("PROCESS_EXECUTIONS_FORBIDDEN"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.code").value("ERRORCODE"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.message").value("Error"))
  ...
  void handleNotAuthorizedExceptionError() throws Exception {
    ...
    performRequest(DATA, MediaType.APPLICATION_JSON)
      ...
      .andExpect(MockMvcResultMatchers.jsonPath("$.category").value("PROCESS_EXECUTIONS_UNAUTHORIZED"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.code").value("ERRORCODE"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.message").value("Error"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.fields").doesNotExist())
      .andExpect(MockMvcResultMatchers.jsonPath("$.traceId").value(traceId));
  ...
  void handleAuthorizationDeniedException() throws Exception {
    ...
    performRequest(DATA, MediaType.APPLICATION_JSON)
      .andExpect(MockMvcResultMatchers.status().isForbidden())
      .andExpect(MockMvcResultMatchers.jsonPath("$.category").value("PROCESS_EXECUTIONS_FORBIDDEN"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.code").value("PROCESS_EXECUTIONS_FORBIDDEN"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.message").value("Error"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.fields").doesNotExist())
      .andExpect(MockMvcResultMatchers.jsonPath("$.traceId").value(traceId));
  }
