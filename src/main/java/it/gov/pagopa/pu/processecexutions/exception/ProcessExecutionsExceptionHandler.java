...
public class ProcessExecutionsExceptionHandler {
  ...
  @ExceptionHandler({InvalidParamException.class, InvalidValueException.class})
  ...
  static ResponseEntity<ProcessExecutionsErrorDTO> handleException(Exception ex, HttpServletRequest request, HttpStatus httpStatus, ProcessExecutionsErrorDTO.CategoryEnum errorEnum) {
    ...
    return ResponseEntity
      ...
      .body(new ProcessExecutionsErrorDTO(errorEnum, code, String.format(ERROR_MESSAGE_FORMAT, code, message), Utilities.getTraceId()));
  }
  ...
  private static Pair<String, String> buildReturnedMessage(Exception ex) {
    switch (ex) {
      case HttpMessageNotReadableException httpMessageNotReadableException -> {
        ...
        return Pair.of(ProcessExecutionsErrorDTO.CategoryEnum.PROCESS_EXECUTIONS_BAD_REQUEST.name(), errorMsg);
      }
      case MethodArgumentNotValidException methodArgumentNotValidException -> {
        return Pair.of(ProcessExecutionsErrorDTO.CategoryEnum.PROCESS_EXECUTIONS_BAD_REQUEST.name(),
      ...
      case DataIntegrityViolationException dataIntegrityViolationException -> {
        ...
        return Pair.of(ProcessExecutionsErrorDTO.CategoryEnum.PROCESS_EXECUTIONS_CONFLICT.name(),
          errorMsg) ;
      }
      case HttpClientErrorException.TooManyRequests tooManyRequestsException -> {
        return Pair.of(ProcessExecutionsErrorDTO.CategoryEnum.PROCESS_EXECUTIONS_TOO_MANY_REQUESTS.name(), tooManyRequestsException.getMessage());
      }
      ...
      default -> {
        if (ex.getCause() instanceof HttpHostConnectException) {
          return Pair.of("PROCESS_EXECUTIONS_CONNECTION_ERROR", ex.getMessage());
        }
